...
def batch_rodrigues(theta):
    ...
    eyes = F.tile(F.expand_dims(Variable(xp.array(xp.eye(3), 'f')), 0), (batch_size, 1, 1))
...
class SMPL(chainer.Chain):
    ...
    def __call__(self, beta, theta, get_skin=False, with_a=False):
        ...
        pose_feature = F.reshape(Rs[:, 1:, :, :] -
                                 F.repeat(F.repeat(Variable(self.xp.array(self.xp.eye(3), 'f'))[None, ], 23, axis=0)[None, ], batch_size, axis=0),
                                 [-1, 207])
